...
$it.name.literal$$if(it.orcid)$~\orcidlink{$it.orcid$}$endif$$if(it.metadata.degrees)$$it.metadata.degrees$$endif$\textsuperscript{$for(it.affiliations)$$it.number$$sep$,$endfor$$if(it.attributes.equal-contributor)$\Yinyang$endif$$if(it.attributes.corresponding)$*$endif$
